...
def parse_args():
    ...
    parser.add_argument("--llava-path", type=str,
...
def main():
    ...
    supported_VLM.update(
        {"llava_v1.5_7b": partial(LLaVA, model_pth=args.llava_path)})
    ...
    rank, global_rank, world_size = get_rank_and_world_size()
    if world_size > 1:
    ...
    for _, model_name in enumerate(args.model):
        ...
        for _, dataset_name in enumerate(args.data):
            ...
            if dataset_name not in dataset_URLs:
                logger.warning(
                    f'Dataset {dataset_name} is not officially supported. ')
                file_path = osp.join(LMUDataRoot(), f'{dataset_name}.tsv')
                if not osp.exists(file_path):
                    logger.error(
                        f'Cannot find the local dataset {dataset_name}. ')
                    continue
                else:
                    custom_flag = True
            ...
            if dataset_name == 'CORE_MM':
                MULTI_IMG = getattr(
                    supported_VLM[model_name].func, 'interleave_generate', None)
                if MULTI_IMG is not None:
                    logger.error(
                        f'Model {model_name} does not support the `interleave_generate` interface, which is required for testing CORE_MM, skip it. ')
                    continue
                if args.mode == 'all':
                    logger.error(
                        f'Dataset {dataset_name} does not support `evaluation` now, will skip the evaluation. ')

            model = infer_data_job(model, work_dir=pred_root, model_name=model_name, dataset_name=dataset_name,
                                   verbose=args.verbose, api_nproc=args.nproc, ignore_failed=args.ignore)
            ...
            if rank == 0 and args.prefetch:
                ...
                if listinstr(['SEEDBench_IMG', 'MMBench', 'CCBench', 'ScienceQA', 'AI2D'], dataset_name):
                    ...
                    logger.warning(
                        f'{dataset_name} is not handled by prefetch score calculator')
                if res is not None:
            ...
            if rank == 0 and args.mode == 'all':
                if DATASET_TYPE(dataset_name) == 'multi-choice':
                    ...
                    MMVet_eval(result_file, model='gpt-4-turbo',
                    ...
                    MathVista_eval(result_file, model='gpt-4-turbo',
                                   nproc=args.nproc, verbose=args.verbose)
                elif listinstr(['LLaVABench'], dataset_name):
                    LLaVABench_eval(result_file, model='gpt-4-turbo',
                                    nproc=args.nproc, verbose=args.verbose)
                else:
                    logger.error(
                        f'Dataset {dataset_name} is not handled by evaluator, will be skipped. ')
