...
def is_v100():
    try:
        result = subprocess.run(
            ['nvidia-smi', '--query-gpu=gpu_name', '--format=csv'], stdout=subprocess.PIPE)
        gpu_name = result.stdout.decode('utf-8').split('\n')[1]
        if 'V100' in gpu_name:
            return True
        else:
            return False
...
def load_pretrained_model(model_path, model_base, model_name, load_8bit=False, load_4bit=False, device_map="auto", device="cuda", **kwargs):
    ...
    if not vision_tower.is_loaded:
        ...
        if os.path.exists(os.path.join(model_path, 'vision_encoder/adapter_config.json')):
            model.get_vision_tower().load_lora(os.path.join(model_path, 'vision_encoder'))
