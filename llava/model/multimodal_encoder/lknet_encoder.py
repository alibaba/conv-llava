...
if __name__ == "__main__":
    ...
    args = Args()
    vision_tower_name = "/mnt/thuair/gcjtcl/models/public/model/CLIP-UniRepLKNet-L-laion5B-s10B-b75k/UniRepLKNet-L-b75k_s10B_CLIP-in1k_75.72.pt"

    # Initialize the LKNetCLIPVisionTower
    vision_tower = LKNetCLIPVisionTower(vision_tower_name, args)

    stage_index = 2
    vision_tower.make_layers_trainable_after_stage(stage_index)
    vision_tower.print_trainable_parameters()

    # for name, param in vision_tower.named_parameters():
    #     print(name)
    #     if len(list(vision_tower.named_parameters())) < 10:
    #         break

    # Test forward method
    images = torch.randn(1, 3, 256, 256)
    output = vision_tower(images)
    print(output.shape)

    vision_tower.config.save_pretrained('.')

    # Test properties
    print(vision_tower.dummy_feature.shape)
