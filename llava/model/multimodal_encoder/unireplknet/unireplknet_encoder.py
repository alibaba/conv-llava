...
def get_conv2d(
    ...
    if attempt_use_lk_impl and need_large_impl:
        ...
        if (
            ...
            print(
                f"===== iGEMM Efficient Conv Impl, channels {in_channels}, kernel size {kernel_size} ====="
            )
...
class UniRepLKNetBlock(nn.Module):
    ...
    def __init__(
        ...
        if self.with_cp:
            print(
                "****** note with_cp = True, reduce memory consumption but may slow down training ******"
            )
    ...
    def reparameterize(self):
        ...
        if hasattr(self.norm, "running_var"):
            ...
            if hasattr(self.dwconv, "lk_origin"):
                ...
                conv.weight.data = self.dwconv.weight * (self.norm.weight / std).view(
                    -1, 1, 1, 1
                )
                conv.bias.data = (
                    self.norm.bias - self.norm.running_mean * self.norm.weight / std
                )
                self.dwconv = conv
            self.norm = nn.Identity()
        if self.gamma is not None:
...
default_UniRepLKNet_S_B_L_XL_kernel_sizes_plus = (
    (3, 3, 3),
    (13, 13, 13),
    (
        13,
...
UniRepLKNet_S_B_L_XL_depths_plus = (3, 3, 27, 3, 6)
...
default_depths_to_kernel_sizes = {
    ...
    UniRepLKNet_S_B_L_XL_depths_plus: default_UniRepLKNet_S_B_L_XL_kernel_sizes_plus,
}
...
class UniRepLKNet(nn.Module):
    ...
    def init_weights(self):
        ...
        if ckpt_path is None:
            print(
                "================ Note: init_cfg is provided but I got no init ckpt path, so skip initialization"
            )
        else:
...
def load_with_key(model, key):
    ...
    if hf_hub_download is not None:
        ...
        cache_file = hf_hub_download(
            repo_id=repo_id, filename=huggingface_file_names[key]
        )
        checkpoint = torch.load(cache_file, map_location="cpu")
    else:
        checkpoint = torch.hub.load_state_dict_from_url(
            url=model_urls[key], map_location="cpu", check_hash=True
        )
...
def initialize_with_pretrained(
    model, model_name, in_1k_pretrained, in_22k_pretrained, in_22k_to_1k
):
    if in_1k_pretrained:
        key = model_name + "_1k"
...
def unireplknet_a(in_1k_pretrained=False, **kwargs):
    model = UniRepLKNet(
        depths=UniRepLKNet_A_F_P_depths, dims=(40, 80, 160, 320), **kwargs
    )
...
def unireplknet_f(in_1k_pretrained=False, **kwargs):
    model = UniRepLKNet(
        depths=UniRepLKNet_A_F_P_depths, dims=(48, 96, 192, 384), **kwargs
    )
...
def unireplknet_p(in_1k_pretrained=False, **kwargs):
    model = UniRepLKNet(
        depths=UniRepLKNet_A_F_P_depths, dims=(64, 128, 256, 512), **kwargs
    )
...
@register_model
def unireplknet_s(
    in_1k_pretrained=False, in_22k_pretrained=False, in_22k_to_1k=False, **kwargs
):
    model = UniRepLKNet(
        depths=UniRepLKNet_S_B_L_XL_depths, dims=(96, 192, 384, 768), **kwargs
    )
    initialize_with_pretrained(
        model, "unireplknet_s", in_1k_pretrained, in_22k_pretrained, in_22k_to_1k
    )
    return model
...
def unireplknet_b(in_22k_pretrained=False, in_22k_to_1k=False, **kwargs):
    model = UniRepLKNet(
        depths=UniRepLKNet_S_B_L_XL_depths, dims=(128, 256, 512, 1024), **kwargs
    )
    initialize_with_pretrained(
        model, "unireplknet_b", False, in_22k_pretrained, in_22k_to_1k
    )
    return model
...
def unireplknet_l(in_22k_pretrained=False, in_22k_to_1k=False, **kwargs):
    model = UniRepLKNet(
        depths=UniRepLKNet_S_B_L_XL_depths, dims=(192, 384, 768, 1536), **kwargs
    )
    initialize_with_pretrained(
        model, "unireplknet_l", False, in_22k_pretrained, in_22k_to_1k
    )
    return model


def unireplknet_l_plus(**kwargs):
    model = UniRepLKNet(
        depths=UniRepLKNet_S_B_L_XL_depths_plus, dims=(192, 384, 768, 1536, 3072), **kwargs
    )
    # initialize_with_pretrained(model, "unireplknet_l", False, False, False)
    return model
...
def unireplknet_xl(in_22k_pretrained=False, in_22k_to_1k=False, **kwargs):
    model = UniRepLKNet(
        depths=UniRepLKNet_S_B_L_XL_depths, dims=(256, 512, 1024, 2048), **kwargs
    )
    initialize_with_pretrained(
        model, "unireplknet_xl", False, in_22k_pretrained, in_22k_to_1k
    )
    return model
...
def unireplknet_h(in_22k_pretrained=False, in_22k_to_1k=False, **kwargs):
    model = UniRepLKNet(
        depths=UniRepLKNet_S_B_L_XL_depths, dims=(480, 960, 1920, 3840), **kwargs
    )
    initialize_with_pretrained(
        model, "unireplknet_h", False, in_22k_pretrained, in_22k_to_1k
    )
    return model
